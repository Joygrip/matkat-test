...
def _current_open_period(db: Session, tenant_id: str):
    """Return (year, month) of the earliest open period.

    Mirrors scheduler._get_open_period: open status defines the active planning
    window.  Falls back to today's calendar month when no open period exists.
    """
    period = (
        db.query(Period)
